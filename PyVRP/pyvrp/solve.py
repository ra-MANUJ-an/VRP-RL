...
class SolveParams:
    ...
    def __init__(
        ...
        custom_selector_path: Optional[Path] = None,  # New parameter
        custom_crossover_path: Optional[Path] = None,  # NEW
        custom_survivor_path: Optional[Path] = None,  # New
    ):
        ...
        self._custom_selector = custom_selector_path
        self._custom_crossover = custom_crossover_path  # NEW
        self._custom_survivor = custom_survivor_path # New
    ...
    @property
    def custom_selector(self) -> Optional[Path]:
        return self._custom_selector

    @property
    def custom_crossover(self) -> Optional[Path]:  # NEW
        return self._custom_crossover

    @property
    def custom_survivor(self): # New
        return self._custom_survivor
    ...
    def from_file(cls, loc: str | pathlib.Path):
        ...
        # New: Load custom selector path
        custom_sel = None
        if "custom_selector" in data:
            custom_sel = Path(data["custom_selector"])

        custom_cross = None  # NEW
        if "custom_crossover" in data:  # NEW
            custom_cross = Path(data["custom_crossover"])  # NEW

        custom_survive = None  # New
        if "custom_survivor" in data:  # New
            custom_survive = Path(data["custom_survivor"])  # New
        ...
        return cls(
            gen_params, pen_params, pop_params, nb_params, node_ops, route_ops, custom_sel, custom_cross, custom_survive  # MODIFIED
        )
...
def solve(
    ...
    # Modified population initialization with LLM support
    pop = Population(
        bpd,
        params.population,
        custom_selector=custom_selector_path or params.custom_selector,
        custom_survivor=params.custom_survivor # New
    )
    ...
    # Configure crossover with proper fallback
    if params.custom_crossover:
        # Correct partial application
        crossover = (
            partial(srex, custom_path=params.custom_crossover) 
            if data.num_vehicles > 1 
            else partial(ox, custom_path=params.custom_crossover)
        )
        # if data.num_vehicles > 1:
        #     crossover = partial(srex, 
        #                       custom_path=params.custom_crossover)
        # else:
        #     crossover = partial(ox, 
        #                       custom_path=params.custom_crossover)
    else:
        crossover = srex if data.num_vehicles > 1 else ox
        # if data.num_vehicles > 1:
